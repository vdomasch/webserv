...
std::string normalize_path(const std::string& path)
{
	...
	if (path.at(path.size() - 1) == '/' && result.size() > 1)
		result += '/';
	return result;
}

std::map<std::string, ServerConfig>::iterator find_server(HttpRequest &req, std::string &server_name, std::map<std::string, ServerConfig> &server_list)
{
	...
	if ((it_serv = server_list.find(server_name)) == server_list.end())
	{
		...
		if ((it_serv = server_list.find(server_name)) == server_list.end())
		{
			...
			build_response(req, 404, "Not Found", "text/html", "404 Server not found", false);
			return it_serv;
		}
	}
	return it_serv;
...
LocationConfig find_location(HttpRequest &req, ServerConfig &server, std::string &location_name, std::string &target, bool &autoindex)
{
...
void	get_request(HTTPConfig &http_config, HttpRequest &req, std::map<std::string, ServerConfig> &server_list, t_fd_data &fd_data, std::string server_name)
{
	...
	std::cout << "Original target: " << target << std::endl;
	std::cout << "Normalized target: " << target << std::endl;
	std::cout << "Server name: " << server_name << std::endl;


	std::map<std::string, ServerConfig>::iterator it_serv = find_server(req, server_name, server_list);
